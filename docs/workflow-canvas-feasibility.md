...
>
> **落地状态（2026-09-11 已实现并验证，交互于 09-12 完善）**：阶段 1（契约统一 + 画布骨架）、阶段 2（11 类节点 + 6 个执行器）、
> 调试（节点级轨迹 + 画布调试面板）、发布（V13 + 版本递增 + 一键回滚）均已完成；
> **143 个测试全绿**，端到端验证通过（顺序链执行 / 调试轨迹 / 发布回滚 / HTTP 节点真实调用 wttr.in）。
> 09-12 补齐 Coze 式交互：**画布左键拖拽平移 / 滚轮缩放、节点整卡拖拽重排、内联「+」精确插入、分支折叠**，
> 内联交互改用官方物料包 `@flowgram.ai/fixed-semi-materials`（原计划自研，见下方记录的调整）。
