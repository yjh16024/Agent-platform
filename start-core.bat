...
REM Prereq: local MySQL running - db agent_platform, user agent
...
REM ---- 1. Locate portable JDK 21 / Maven under tools ----
set "JAVA_HOME="
if exist "%USER_HOME%\tools\jdk21\jdk-21.0.12.1+1" set "JAVA_HOME=%USER_HOME%\tools\jdk21\jdk-21.0.12.1+1"
if defined JAVA_HOME set "PATH=%JAVA_HOME%\bin;%PATH%"
if not defined JAVA_HOME echo [WARN] portable JDK 21 not found, falling back to system java
...
if defined REDIS_HOME (
    ...
    echo [INFO] portable Redis not found under tools\redis, skip.
    echo [INFO] health check will show redis DOWN; core features still work.
)
